...
int main()
{
	...
	std::pair<char *, size_t> ret = load("../cmake-build-debug/a.msi");

	char *buf = ret.first;
	size_t file_size = ret.second;
	std::println("Start!");
	...
	while (true)
	{
		...
		data.array = buf;
		std::tuple<int, int, array_with_size<4088, const char*>> data_pkt = {4088, 2, data};
